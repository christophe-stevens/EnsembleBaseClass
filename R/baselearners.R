# chris-change: Add not in operator
"%nin%" <- Negate("%in%")
...
# chris-change: added a classification config base class
setClass("Classification.Config", contains="BaseLearner.Config")
...
make.configs <- function(baselearner=c("nnet","rf","svm","gbm","knn", "penreg"), config.df, type="regression") {
  ...
  if (length(baselearner)==1) {
    ...
    if (missing(config.df))
     ret <- lapply(baselearner, FUN = function(x) make.configs(baselearner=x,type=type))
...
# chris-change: adding classification but not sure it is needed to be honest
Classification.CV.Fit <- function(classification.config, formula, data, partition, tmpfiles=NULL, print.level=1) {
  nfolds <- max(partition)
  if (length(partition)!=nrow(data)) stop("length of fold parameter does not match number of rows in data")
  pred <- rep(NA, nrow(data))
  reg.list <- list()
  for (i in 1:nfolds) {
    ...
    data_train <- data[-index_predict,]
    data_predict <- data[index_predict,]
    regtmp <- BaseLearner.Fit(object=classification.config, formula=formula, data=data_train, tmpfile=tmpfiles[i], print.level=print.level-1)
    pred[index_predict] <- predict(regtmp, newdata=data_predict)
    regtmp@pred <- NULL # cv object has the full pred, so we don't need partial copies from each fold
    reg.list[[i]] <- regtmp
    flush.console()
  }
  ret <- Classification.CV.FitObj(fitobj.list=reg.list, pred=pred, partition=partition, filemethod=!is.null(tmpfiles))
  return (ret)
}
...
# chris-change: Adding classification
predict.Classification.CV.FitObj <- function(object, newdata=NULL, ...) {
  if (is.null(newdata)) return (object@pred)
  nfolds <- length(object@fitobj.list)
...
Classification.CV.Batch.Fit <- function(instance.list, formula, data, ncores=1, filemethod=FALSE, print.level=1
  ...
  if (ncores==1) { # serial execution
    ret <- lapply(1:njobs, function(i) {
      ...
      flush.console()
      t <- proc.time()[3]
      rettmp <- Classification.CV.Fit(classification.config=myconfig, formula=formula, data=data
                                  , partition=instance.list@partitions[,instance.list@instances[[i]]@partid]
                                  , tmpfiles=tmpfiles[index.list$start[i]:index.list$end[i]]
                                  , print.level=print.level-1)
      t <- proc.time()[3] - t
  ...
  return (Classification.CV.Batch.FitObj(fitobj.list=ret, instance.list=instance.list
                                     , filemethod=!is.null(tmpfiles), tmpfiles=tmpfiles, tmpfiles.index.list=index.list, tvec = tvec, pred=pred, y=y))
}
...
predict.Classification.CV.Batch.FitObj <- function(object, ..., ncores=1
                                               , preschedule = TRUE) {
  if (ncores==1) {
    predmat <- sapply(object@fitobj.list, function(x) {
      predict(x, ...)
...
# chris-change: adding for classification
plot.Classification.CV.Batch.FitObj <- function(x, errfun=rmse.error, ylim.adj = NULL, ...) {
  configs <- sapply(x@instance.list@instances, function(instance) instance@config)
  baselearner.names <- sapply(configs, extract.baselearner.name, "classification")
  baselearner.names.unique <- unique(baselearner.names)
  n.tot <- length(baselearner.names)
  n.unique <- length(baselearner.names.unique)
  y <- x@y
...
# chris-change: adding for classification
setMethod("validate", "Classification.CV.Batch.FitObj"
          , function(object, newdata, formula, errfun=rmse.error, make.plot=TRUE) {
            if (missing(newdata)) stop("must provide newdata argument")
            ...
            ret <- list(errors=error, newerrors=newerror, corr=corr, corr.vec=corr.vec, baselearners=baselearner.names.unique)
            class(ret) <- "validate.Classification.Batch.FitObj"
            return (ret)
          })
...
Classification.Batch.Fit <- function(config.list, formula, data, ncores=1, filemethod=FALSE, print.level=1) {
  ...
  if (ncores==1) {
    ...
    # to improve performance of dynamic scheduling, sort in descending order of expected execution time
    registerDoParallel(ncores)
    # use function as global in threads
    ret <- foreach(ii=1:njobs, .options.multicore=list(preschedule=FALSE)) %dopar% {
      #source("./R/thread_util.R")
      myconfig <- config.list[[ii]]
      baselearner.name <- extract.baselearner.name(myconfig, type="classification" )
  ...
  return (Classification.Batch.FitObj(fitobj.list=ret, config.list=config.list
                                  , filemethod=!is.null(tmpfiles), tmpfiles=tmpfiles, pred=pred, y=y))
}


# chris-change: adding for classification
predict.Classification.Batch.FitObj <- function(object, ..., ncores=1) {
  if (ncores==1) {
    predmat <- sapply(object@fitobj.list, function(x) {
      predict(x, ...)
    })
...
# chris-change: adding for classification
plot.Classification.Batch.FitObj <- function(x, errfun=rmse.error, ...) {
  baselearner.names <- sapply(x@config.list, extract.baselearner.name, "classification")
  baselearner.names.unique <- unique(baselearner.names)
  n.tot <- length(baselearner.names)
  n.unique <- length(baselearner.names.unique)
  y <- x@y
...
# chris-change: adding for classification
# chris-change: adding for classification
setMethod("validate", "Classification.Batch.FitObj"
          , function(object, newdata, formula, errfun=rmse.error, make.plot=TRUE) {
            if (missing(newdata)) stop("must provide newdata argument")
            
            baselearner.names <- sapply(object@config.list, extract.baselearner.name, "classification")
            baselearner.names.unique <- unique(baselearner.names)
            n.tot <- length(baselearner.names)
            n.unique <- length(baselearner.names.unique)
            y <- object@y
...
# chris-change: adding for classification
print.validate.Classification.Batch.FitObj <- function(x, ...) {
  cat("average correlation between training and hold-out errors:", x$corr, "\n")
  cat("error correlation by base learner:\n")
  cat(paste(paste0(format(x$corr.vec, digits=3), " (", x$baselearners, ")"), collapse=", "), "\n")
}
